...
ImagePPM::ImagePPM(Pixel** new_pixels,
                   int new_height,
                   int new_width,
                   int new_k_max_color):
    height_(new_height),
    width_(new_width),
    max_color_value_(new_k_max_color),
    pixels_(new_pixels) {}
...
std::ostream& operator<<(std::ostream& os, const ImagePPM& image) {
  ...
  for (int row = 0; row < image.height_; row++) {
    for (int col = 0; col < image.width_; col++) {
      int green = image.pixels_[row][col].GetGreen();
      int red = image.pixels_[row][col].GetRed();
      int blue = image.pixels_[row][col].GetBlue();
      os << red << std::endl;
      os << green << std::endl;
      os << blue << std::endl;
    }
...
void ImagePPM::ChangeVerticalImage(Pixel**& new_pixels,
                                   int new_height,
                                   int new_width) {
  Clear();
  pixels_ = new_pixels;
...
void ImagePPM::ChangeHorizontalImage(Pixel**& new_pixels,
                                     int new_height,
                                     int new_width) {
  Clear();
  pixels_ = new_pixels;
